...
async def create_user(db: Session, user: user_schemas.UserCreate):
...
async def ban(id: int, db: Session):
    info_about_user(db=db, id=id)
    interesting_user = db.query(models.User).\
        filter(models.User.id == id).\
        filter(models.User.active is True)
    if interesting_user:
...
async def unban(id: int, db: Session):
    info_about_user(db=db, id=id)
    interesting_user = db.query(models.User).\
        filter(models.User.id == id).\
        filter(models.User.active is False)
    if interesting_user:
