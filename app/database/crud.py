...
def create_advertisement(db: Session, adv: adv_schemas.AdvIn, user_id: int, current_user):
...
def get_all_adv(db: Session, current_user):
    get_adv = db.query(models.Advertisements).all()
    return get_adv



def info_about_adv(db: Session, id: int, current_user):
...
def delete_adv(id: int, user_id: int, db: Session, current_user):
...
def post_a_feedback(feedback, user_id: int, db: Session, current_user):
    find_feedback = db.query(models.Feedback).\
...
def ban(id: int, db: Session, current_user):
